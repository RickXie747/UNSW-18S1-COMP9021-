...
'''
此版本根本未完成
get_triangle_n为查找固定size三角形的方法
get_triangle_n1为扩展遍历版本，对于固定顶点，找所有size的三角形
'''

def get_triangle_n(size, count = 0):
    for i in range(0, dim - (size - 1)):
        for j in range((size - 1), dim - (size - 1)):
            if grid[i][j] == 0:
                continue
            if not grid[i + 1][j - 1: j + 2].count(0):
                count += 1
    return count
