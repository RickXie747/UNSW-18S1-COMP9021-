...
def is_word_word_ladder(word_1, word_2, candidate_ladder):
    ...
    for e in candidate_ladder:
        if e not in lexicon:
            return False
    if len(candidate_ladder) == 1:
        if word_1 == word_2 == candidate_ladder[0]:
            return True
        else:
            return False
    if len(candidate_ladder) == 2:
        ...
        for i in range(len(candidate_ladder) - 1):
            if candidate_ladder[i + 1] not in closest_words[candidate_ladder[i]]:
                return False
        for a in candidate_ladder:
            for b in candidate_ladder:
                if a == b:
                    continue
                if a in closest_words[b]:
                    count += 1
        if count//2 == len(candidate_ladder) - 1:
            return True
        return False
