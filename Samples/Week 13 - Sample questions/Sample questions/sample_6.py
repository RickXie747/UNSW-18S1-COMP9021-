    ...
    try:
        ...
        for e in stack:
            if e in operators:
                count_o += 1
                continue
            if e[0] in ['+', '-'] and e[1:].isdigit:
                count_d += 1
                continue
            else:
                e = int(e)
                count_d += 1
        if count_o != count_d - 1:
            raise ListNonEmpty       
...
def evaluate_prefix_expression(expression):
    ...
    for i in range(len(stack)):     
        if stack[i][0] in ['+', '-'] and len(stack[i]) >= 2:
            stack[i] = int(stack[i][1:]) if stack[i][0] == '+' else -int(stack[i][1:])
    ...
    for i in range(len(stack)):     
        if stack[i] == '*':
            ...
            stack[i + 1], stack[i + 2] = 'x', 'x'

    for i in range(len(stack)):
        if stack[i] not in ['+', '-', '/']:
            flag = False
        if stack[i] == '+' and not flag: 
            stack[i], stack[i + 1], stack[i + 2] = stack[i + 1] + stack[i + 2], 'x', 'x'
        if stack[i] == '-' and not flag: 
            stack[i], stack[i + 1], stack[i + 2] = stack[i + 1] - stack[i + 2], 'x', 'x'            
        if stack[i] == '/' and not flag: 
            stack[i], stack[i + 1], stack[i + 2] = stack[i + 1] / stack[i + 2], 'x', 'x'
    ...
    while stack:
        if type(stack[-1]) is int:
            stack_d.append(stack.pop())
        else:
            stack_o.append(stack.pop())
    stack_o = stack_o[::-1]

    while stack_o:
        if stack_o[-1] == '+':
            stack_d.append(stack_d.pop() + stack_d.pop())
        elif stack_o[-1] == '-':
            stack_d.append(stack_d.pop() - stack_d.pop())
        elif stack_o[-1] == '/':
            stack_d.append(stack_d.pop() / stack_d.pop())
